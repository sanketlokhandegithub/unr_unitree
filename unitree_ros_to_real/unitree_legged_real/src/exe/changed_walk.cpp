...
int mainHelper(int argc, char *argv[], TLCM &roslcm)
{
    ...
    while (ros::ok()){
        ...
        if(motiontime>1000 && motiontime<1500){
            SendHighROS.mode = 1; //Mode 1 is stationary body mvmt?
            SendHighROS.roll = 0.3f;
            // SendHighROS.forwardSpeed = 0.1f; //Think we need mode 2 for this
        ...
        if(motiontime>1500 && motiontime<2000){
            ...
            // SendHighROS.forwardSpeed = 0.0f; //Mode 2 function
        ...
        if(motiontime>5000 && motiontime<8500){
            SendHighROS.mode = 2; //Mode 2 seems to be walk
